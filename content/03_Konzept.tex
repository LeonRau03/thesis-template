...
\midrule
Frontend & Angular~19, Nginx & Einstiegspunkt; Nginx routet an Backend (\texttt{/api}), WS-Gateway (\texttt{/gateway}), Keycloak (\texttt{/auth}), AI Provider (\texttt{/ai}) \\
Backend & Spring Boot 3.1.5, WebFlux, R2DBC & Zentraler Dienst: 95~Endpunkte, 19~Controller; Single Point of Failure \\
WS-Gateway & Kotlin, Netty, STOMP & Echtzeit-Updates via RabbitMQ; Session-Tracking in-memory \\
AI Provider & Python 3.12, FastAPI, LangChain & 19~LLM-Provider; externe Abhängigkeiten ohne Timeout \\
\bottomrule
\end{tabularx}
\end{table}

Die Infrastruktur umfasst die PostgreSQL-Hauptdatenbank (Version~12.1), eine dedizierte Keycloak-Datenbank (PostgreSQL~15), eine AI-Datenbank (PostgreSQL~17) mit logischer Replikation, RabbitMQ~3.8 als Message Broker, Keycloak~22 für Authentifizierung (OIDC/SSO) und einen E-Mail-Dienst. Für die Observability-Strategie ist die Deployment-Topologie wesentlich: Alle Container laufen auf einem einzelnen Host ohne konfigurierte Ressourcenlimits (CPU, Speicher) und ohne Health-Check-Direktiven auf Container-Ebene.
 
\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{images/C4-Container-Diagram_fragjetzt.png}
\caption{C4-Container-Diagramm der frag.jetzt-Architektur}
\label{fig:c4-fragjetzt-architektur}
\end{figure}

Eine Vollseitenansicht des Diagramms (um 90\textdegree{} gedreht) befindet sich in Anhang~\ref{sec:anhang-c4-vollansicht}, Abbildung~\ref{fig:c4-fragjetzt-architektur-vollseite}.
 
 
\section{Kritische User Journeys}
 
Eine wirksame Observability-Strategie orientiert sich an den tatsächlichen Nutzungspfaden, nicht an abstrakten Systemkomponenten (vgl. Niedermaier et al., 2019, S.~10--11). Für frag.jetzt lassen sich drei geschäftskritische Abläufe identifizieren.
...
Ergänzend kann synthetisches End-to-End-Monitoring diese Journeys von außen überwachen und Verfügbarkeit sowie Antwortverhalten aus Nutzersicht validieren (vgl. Niedermaier et al., 2019, S.~11).
...
Weder Backend noch AI Provider konfigurieren Timeouts, Retries oder Circuit Breaker auf ausgehenden HTTP-Aufrufen. Ein nicht reagierender externer Dienst kann Request-Laufzeiten unbegrenzt verlängern und die Ressourcenauslastung erhöhen. Niedermaier et al. ordnen fehlende Transparenz über Drittanbieter-Abhängigkeiten als zentrale betriebliche Herausforderung verteilter Systeme ein (vgl. Niedermaier et al., 2019, S.~8--9).
 
\subsection{R2: Datenbankengpässe bei Massenabstimmungen.}
Die Vote-Trigger führen bei jedem Schreibvorgang einen zusätzlichen \texttt{SELECT} und \texttt{UPDATE} auf der Comment-Tabelle aus; bei gleichzeitigen Abstimmungen entsteht Row-Level Contention. Die Kommentarnummern-Generierung skaliert zudem sublinear mit wachsender Kommentarzahl. Fehlende Indizes auf mehreren neueren Tabellen verschärfen das Problem.
...
\begin{table}[ht]
\centering
\caption{Zuordnung der Risikopunkte zu Golden Signals und Nutzerwirkung}
\label{tab:risiken}
\small
