...
Sämtliche Panels verwenden \texttt{\$\_\_rate\_interval} als dynamisches Zeitfenster für Rate-Berechnungen. Die vollständige JSON-Definition befindet sich in Anhang~ref{app:grafana-json}.
...
Die Evaluationsfenster von drei bis fünf Minuten fungieren als Debounce-Mechanismus, der transiente Spitzen toleriert \parencite[S.~176]{vinnakota2025creating}. Jede Regel enthält in ihrer Annotation-Konfiguration einen Link zum zugehörigen Runbook sowie zum relevanten Dashboard-Panel. Die drei Regeln wurden im Prototyp angelegt und konfiguriert. Ihre Verifikation erfolgte durch gezielte Belastungsszenarien auf dem Staging-Server und wird in Kapitel~8 dokumentiert.

Ergänzend wurden drei Runbooks als strukturierte Dokumente erstellt, die der Fünf-Felder-Struktur (Symptom, Kontext, Diagnose, Behebung, Eskalation) aus Kapitel~6.6 folgen. Das erste behandelt den Alarm ``Erhöhte Backend-Latenz'', das zweite den Alarm ``Backend-Fehlerrate über Schwellenwert'' und das dritte den Alarm ``Infrastruktur-Sättigung'' Die Runbooks befinden sich im Anhang~ref{app:runbooks}.
