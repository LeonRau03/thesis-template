...
Ein Nutzer erstellte über die Weboberfläche eine neue Frage in einem aktiven Raum. Erwartet wurde ein Root-Span für den \texttt{POST /comment/}-Endpunkt mit untergeordneten Spans für den Datenbank-Insert sowie ein Anstieg des Latenz- und Traffic-Panels im Dashboard. In Grafana war der entsprechende Trace mit 8~Spans sichtbar, darunter der \texttt{INSERT}-Span auf der PostgreSQL-Datenbank mit einer Dauer von 3,2\,ms. Das Latenz-Panel im Dashboard zeigte den Request im p50-Bereich bei circa 12\,ms. Dieses Ergebnis bestätigt, dass die in A1 geforderte Nachvollziehbarkeit für die instrumentierten Dienste funktional gegeben ist.
...
Einschränkend ist zu vermerken, dass zwar die drei Regeln im Prototyp angelegt und konfiguriert wurden und eine erste Störungsinjektion den 5xx-Alert auslöste, die kurzfristige Fehlerdiagnostik jedoch dennoch eingeschränkt blieb, weil die zugehörigen Fehler-Metriken in kurzen Zeitfenstern keine konsistent aktuellen Werte lieferten.
