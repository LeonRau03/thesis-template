...
Host & Node Exporter, cAdvisor & CPU, Speicher, Netzwerk, Container-Metriken & Kapazitätsanzeige im Dashboard mit Schwellenwertfärbung \\
\bottomrule
\end{tabularx}
\end{table}
...
Darüber hinaus visualisiert der Service Graph in Grafana (siehe Abbildung~\ref{fig:tempo-service-graph}) die Kommunikationsbeziehungen zwischen den instrumentierten Diensten. Im Prototyp zeigt dieser aktuell drei Knoten (\texttt{fragjetzt-backend}, \texttt{fragjetzt-ws-gateway} und \texttt{fragjetzt-postgres}) mit ihren jeweiligen Anfrageraten und Antwortzeiten.
...
Das Dashboard enthält 17~Panels in fünf Sektionen, die in fünf Sektionen unterteilt sind und die Signal-Matrix aus Kapitel~6.3 visuell umsetzen. Die Sektionen \emph{Latency} und \emph{Traffic} sind beispielhaft in Abbildung~\ref{fig:grafana-dashboard-latency-traffic} dargestellt. Im Bereich \emph{Latency} werden die Backend-Antwortzeiten als Perzentil-Zeitreihen (p50, p95, p99) sowie eine aufgeschlüsselte Latenzansicht pro Endpunkt, basierend auf der Metrik \texttt{http\_server\_request\_duration\_seconds\_bucket} visualisiert. Die Sektion \emph{Traffic} stellt die Anfragerate pro Dienst aus Span-Metriken sowie die meistfrequentierten Endpunkte als gestapeltes Balkendiagramm dar. Die Sektion \emph{Errors} kombiniert eine Span-basierte Fehlerrate pro Service mit einem numerischen Indikator für die HTTP-5xx-Quote. Wie in Abbildung~\ref{fig:grafana-dashboard-errors} ersichtlich, werden zusätzlich die fehlerhäufigsten Span-Operationen tabellarisch aufgeführt. Die Sektion \emph{Saturation} umfasst JVM-Heap-Auslastung, Thread-Zähler und CPU-Nutzung als Kapazitätsanzeige mit Schwellenwertfärbung. Die \emph{Infrastruktur}-Sektion zeigt PostgreSQL-Verbindungen, RabbitMQ-Queue-Tiefen, Host-Ressourcen, Garbage-Collection-Pausen und den Service Graph aus Tempo.
...
Sämtliche Panels verwenden \texttt{\$\_\_rate\_interval} als dynamisches Zeitfenster für Rate-Berechnungen. Die vollständige JSON-Definition befindet sich in Anhang~\ref{app:dashboard}.


\section{Alerting-Regeln und Beispiel-Runbooks}

Die Alerting-Logik aus Kapitel~6.5 wird exemplarisch in drei Grafana-Alerting-Regeln überführt, die Anforderung~A4 adressieren.
...
Die Evaluationsfenster von drei bis fünf Minuten fungieren als Debounce-Mechanismus, der transiente Spitzen toleriert \parencite[S.~176]{vinnakota2025creating}. Jede Regel enthält in ihrer Annotation-Konfiguration einen Link zum zugehörigen Runbook. Die drei Regeln wurden im Prototyp angelegt und konfiguriert. Ihre Verifikation erfolgte durch gezielte Belastungsszenarien auf dem Staging-Server und wird in Kapitel~8 dokumentiert.

Ergänzend wurden drei Runbooks als strukturierte Dokumente erstellt, die der Fünf-Felder-Struktur (Symptom, Kontext, Diagnose, Behebung, Eskalation) aus Kapitel~6.5 folgen. Das erste behandelt den Alarm ``Erhöhte Backend-Latenz'', das zweite den Alarm ``Backend-Fehlerrate über Schwellenwert'' und das dritte den Alarm ``Infrastruktur-Sättigung'' Die Runbooks befinden sich im Anhang~\ref{app:runbooks}.
