...
% [HIER MÖGLICHES BILD EINFÜGEN] Logische Zielarchitektur der Observability-Pipeline für frag.jetzt -- 
% Schichtenmodell mit Instrumentierung (OTel Agents, Exporter), Sammlung (OTel Collector, Prometheus Scrape), 
% Speicherung (Prometheus, Loki, Tempo) und Auswertung (Grafana), eingebettet in die Docker-Compose-Topologie.]
 
Die beschriebene Architektur legt fest, \emph{wie} Telemetriedaten fließen. Im nächsten Abschnitt wird darauf aufbauend bestimmt, \emph{welche} konkreten Beobachtungsgrößen an den einzelnen Diensten erhoben werden.
...
Die vorangehenden Abschnitte haben die Golden Signals je Komponente beschrieben. Für den operativen Einsatz in Dashboards und Alerting ist eine kompakte Gesamtübersicht erforderlich, die auf einen Blick zeigt, welche Messgröße an welcher Stelle erhoben wird, welchem Zweck sie dient und welche Rolle primär von ihr profitiert. Tabelle~\ref{tab:signal-matrix} stellt diese Informationen als Signal-Matrix zusammen.
 
\begin{table}[htbp]
\centering
\caption{Serviceübergreifende Signal-Matrix: Zuordnung von Dienst, Golden Signal, konkreter Messgröße, Erhebungszweck und primärer Stakeholder-Rolle.}
