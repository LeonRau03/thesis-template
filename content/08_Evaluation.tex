...
A4 & Alle drei Alerts in Belastungsszenarien ausgelöst; Multi-Signal-Logik validiert & Kurzfristige Fehler-Metriken lieferten keine konsistent aktuellen Werte; Sättigungs-Alert mit angepasstem Schwellenwert getestet & weitgehend erfüllt \\[4pt]
...
Von den sechs identifizierten Risikopunkten werden R2 (DB-Engpässe), R3 (Intransparenz des Backends) und R5 (Ressourcenverdrängung) durch den Prototyp direkt adressiert. R3 wurde durch die Einführung von Distributed Tracing im instrumentierten Backend-Pfad substanziell reduziert. R4 (WebSocket-Verbindungsverlust) wird auf Beobachtungsebene teilweise adressiert, weil RabbitMQ-Queue-Tiefen und das WS-Gateway sichtbar gemacht werden. Der zugrunde liegende Ausfallpfad bleibt jedoch bestehen. R1 (fehlende Timeouts und Resilienz) sowie R6 (Single-Host-Topologie als systemweiter Ausfallpfad) bleiben als architektonische Defizite bestehen, die zwar durch Observability sichtbar, aber nicht durch den Prototyp selbst behoben werden.

\paragraph{Umsetzungsrealismus.}
Die praktische Implementierung des Observability-Stacks erwies sich als architektonisch komplexer und zeitaufwendiger als initial angenommen. Insbesondere die korrekte Konfiguration der Telemetrie-Pipeline, die feingranulare Abstimmung der verschiedenen Speicher-Backends sowie das Ressourcenmanagement auf dem Single-Host-System erforderten einen erheblichen iterativen Aufwand. Diese unvorhergesehene Komplexität war der primäre Grund für die in Abschnitt~7.1 vorgenommene Abgrenzung des Prototyps. Um eine funktionsfähige und stabile Kerninfrastruktur zu gewährleisten, musste der Implementierungsumfang bewusst reduziert werden. Dies führt dazu, dass nicht alle gestellten Anforderungen im Rahmen dieser Arbeit vollständig erfüllt werden konnten.
...
\emph{Drittens, eingeschränkte Aussagekraft der Rate-Berechnung.} Bei kurzen Betrachtungszeiträumen lieferte die PromQL-Variable \texttt{\$\_\_rate\_interval} inkonsistente Ergebnisse für die HTTP-5xx-Rate. In längeren Zeitfenstern (ab circa 12~Stunden) waren die Werte korrekt. Diese Einschränkung betrifft die Dashboard-Darstellung, nicht die Alerting-Regeln mit festen Zeitfenstern.
